...
def convert_field_types(document):
    ...
    if 'cart_products' in document and isinstance(document['cart_products'], list):
        for product in document['cart_products']:
            if 'option' in product and not isinstance(product['option'], list):
                product['option'] = []

    if 'option' in document and not isinstance(document['option'], list):
        document['option'] = [document['option']]
    elif 'option' not in document:
        document['option'] = []
    return document
...
input_file = 'glamira_ubl_oct2019_nov2019.json'
output_file = 'summary.json'

start_time = time.time()
total_records = remove_id_and_convert_types(input_file, output_file)
end_time = time.time()
